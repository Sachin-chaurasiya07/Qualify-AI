import React, { useState } from "react";
import { FaArrowLeft, FaCheckCircle } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import {motion} from 'motion/react';
import axios from "axios";
import { ServerUrl } from "../App";
import { useDispatch } from "react-redux";
import { setUserData } from "../redux/userSlice";


function Pricing(){

    const navigate= useNavigate();
    const [selectedPlan , setSelectedPlan] = useState('free');
    const [loadingPlan, setLoadingPlan] = useState(null);
    const dispatch = useDispatch();

    const Plans = [{
        id:"free",
        name: "free",
        price :"Rs.0",
        credits :100,
        description : "Perfect for beginners starting interview preparation",
        features: [
            "100 AI interview credits",
            'Basic performance Report',
            'Voice Interview Access',
            'Limited History Access',
        ],
        default : true,
    },
    {
        id: "basic",
        name : "Starter pack",
        price : "Rs. 100",
        credits : 150,
        description : "Great for interviewees looking to practice and improve",
        features : [
            "150 AI interview credits",
            'Detailed Performance Report',
            'Voice Interview Access',
            'full history access',

        ],
        badge : "Most Popular",
        default : false,
    },
    {
        id: "pro",
        name : "Pro pack",
        price : "Rs. 200",
        credits : 300,
        description : "Best for serious candidates aiming for top-tier positions",
        features : [
            "300 AI interview credits",
            'Comprehensive Performance Report',
            'Voice Interview Access',
            'full history access',
            'Priority Support'

        ],
        badge : "Best Value",
        default : false,
    }
]



    const  handlePayment = async(plan)=>{
        try {
            setLoadingPlan(plan.id)
            const amount = plan.id === "basic" ? 100 : plan.id === "pro" ? 200 : 0;
            const result = await axios.post(ServerUrl + "/api/payment/order", {
                planId : plan.id,
                amount,
                credits : plan.credits,
            }, {withCredentials:true})
            console.log(result.data)
            const options = {
                key : import.meta.env.VITE_RAZORPAY_KEY_ID,
                amount : result.data.amount,
                currency : "INR",
                name : "Interview Buddy",
                description : `Payment for ${plan.name} - ${plan.credits} credits`,
                order_id : result.data.id,

                handler : async (response)=>{
                    const verifyPay = await axios.post(ServerUrl + "/api/payment/verify", response, {withCredentials:true})
                    dispatch(setUserData(verifyPay.data.user));
                    alert ("Payment successful! Your credits will be updated shortly.");
                    navigate("/");

            }
        ,
                theme : {
                    color : "#10b981",
                }, 

            };

            const rzp = new window.Razorpay(options);
            rzp.open();
            setLoadingPlan(null)    
            
        } catch (error) {
            console.log(error)
            setLoadingPlan(null)
            
        }

    }
    return(
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-emerald-100 py-16 px-6">
            <div className="max-w-6xl mx-auto mb-14 flex items-start gap-4">
                <button 
                onClick={()=>navigate('/')}
                className="mt-2 p-3 rounded-full bg-white shadow hover:shadow-md transition">
                    <FaArrowLeft className="text-gray-600"/> 

                </button>
                <div className="text-center w-full">
                    <h1 className="text-4xl font-bold text-gray-800">
                        Choose Your Plan
                    </h1>
                    <p className="text-gray-500 mt-3 text-lg">
                        Flexible pricing to match your interview preparation goals
                    </p>
                </div>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">

                {
                    Plans.map((Plans)=>{
                        const isSelected  = selectedPlan === Plans.id

                        return(
                            <motion.div key={Plans.id}
                            whileHover={!Plans.default && {scale:1.03}}
                            onClick={()=>!Plans.default && setSelectedPlan(Plans.id)}
                            className={`relative rounded-3xl p-8 transition-all duration-300 border ${
                                isSelected ? 'border-emerald-600 shadow-2xl bg-white' : 'border-gray-200 bg-white shadow-md'
                            }
                            ${Plans.default ? "cursor-default" : 'cursor-pointer'}
                            `}>
                                {
                                    Plans.badge && (
                                        <div className="absolute top-6 right-6 bg-emerald-600 text-white text-xs px-4 py-1 rounded-full shadow">
                                            {Plans.badge}
                                        </div>
                                    )
                                }
                                {
                                    Plans.default && (
                                        <div className="absolute top-6 right-6 bg-gray-200 text-gray-700 tex-xs px-3 py-1 rounded-full">
                                            Default
                                        </div>
                                    )
                                }
                                {
                                    <h3 className="text-xl font-semibold text-gray-800">
                                        {Plans.name}
                                    </h3>
                                }
                                <div className="mt-4">
                                    <span className="text-3xl font-bold text-emerald-600">
                                        {Plans.price}
                                    </span>
                                    <p className="text-gray-500 mt-1">
                                        {Plans.credits} credits
                                    </p>
                                </div>
                                <p className="text-gray-600 mt-4 text-sm leading-relaxed">
                                    {Plans.description}
                                </p>
                                <div className="mt-6 space-y-3 text-left">
                                    {
                                        Plans.features.map((features, i )=>(
                                            <div key={i} className="flex items-center gap-3">
                                                <FaCheckCircle className="text-emerald-500 text-sm" />
                                                <span className="text-gray-700 text-sm">
                                                    {features}
                                                </span>
                                            </div>
                                        ))
                                    }
                                </div>
                                {!Plans.default &&
                                    <button
                                    disabled={loadingPlan=== Plans.id}
                                    onClick={(e)=>{e.stopPropagation() ;
                                        if(!isSelected){ setSelectedPlan(Plans.id);}
                                        else {handlePayment(Plans)} }}
                                    className={`w-full mt-8 py-3 rounded-xl font-semibold transition ${isSelected ? "bg-emerald-600 text-white hover:opacity-90" : "bg-gray-100 text-gray-700 hover:bg-emerald-50"}`}>
                                        {loadingPlan=== Plans.id ? "Processing..." : isSelected ? "Proceed to pay" : "Select plan"}
                                    </button>
                                }
                            </motion.div>
                        )
                    })
                }
            </div>

        </div>
    )
}

export default Pricing